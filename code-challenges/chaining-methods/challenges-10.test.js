...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let output = input.map(value =>{
    return value.filter(val2 => typeof val2 === 'number'&& val2%5 === 0);
  });

  let output2 = output.map(val3 =>{
    return val3.map( val4 => Math.pow(2,val4));
  });
  return output2;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let filterObj = data.filter( value => value.gender === 'male'||value.gender === 'female');
  let output = filterObj.map(value2 => value2.name);
...
let findShortest = (data) => {
  ...
  let output = '';
  data.map( val => {
    if( !output || val.name.length <= output.length ){
      output=val.name;
    }
  });
  return output;
